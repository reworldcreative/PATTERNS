...
interface PlaneFactory {
  createPlane(manufacturer: string, model: string): Plane;
}

class PassengerPlaneFactory implements PlaneFactory {
  createPlane(manufacturer: string, model: string): Plane {
    return new PassengerPlane(manufacturer, model);
  }
}

class CargoPlaneFactory implements PlaneFactory {
  createPlane(manufacturer: string, model: string): Plane {
    return new CargoPlane(manufacturer, model);
  }
}

const passengerPlaneFactory: PlaneFactory = new PassengerPlaneFactory();
const passengerPlane: Plane = passengerPlaneFactory.createPlane(
  "Boeing",
  "747"
);
passengerPlane.takeOff(); // Passenger plane Boeing 747 is taking off.
...
const cargoPlaneFactory: PlaneFactory = new CargoPlaneFactory();
const cargoPlane: Plane = cargoPlaneFactory.createPlane("Airbus", "A380");
cargoPlane.takeOff(); // Cargo plane Airbus A380 is taking off.
cargoPlane.land(); // Cargo plane Airbus A380 is landing.
